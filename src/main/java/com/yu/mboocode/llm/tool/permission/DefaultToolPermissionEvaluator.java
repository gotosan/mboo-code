...
public class DefaultToolPermissionEvaluator implements ToolPermissionEvaluator {
    ...
    public ToolPermissionChain evaluate(String sessionId, ToolExecutionRequest request, ToolPermissionSpec spec) {
        ...
        if (type == ToolPermissionType.TOOL) {
            PermissionCheck check = permissions.getAllowedTools().contains(request.name()) ? PermissionCheck.allowed() : PermissionCheck.needAsk();
            return new ToolPermissionChain(List.of(new PermissionRequirement(type, null, request.name(), spec.title(), spec.description(), check)));
        } else if (type == ToolPermissionType.READ || type == ToolPermissionType.WRITE) {
            return new ToolPermissionChain(List.of(evaluatePath(request, spec, session, permissions)));
        }
        PermissionCheck check = PermissionCheck.error(ToolPermissionErrorCode.PERMISSION_UNKNOWN_TYPE, "未知的工具权限类型");
        return new ToolPermissionChain(List.of(new PermissionRequirement(type, null, request.name(), spec.title(), spec.description(), check)));
    }

    private PermissionRequirement evaluatePath(ToolExecutionRequest request, ToolPermissionSpec spec, Sessions session, SessionPermissions permissions) {
        try {
            ...
            PermissionCheck check = isPathAuthorized(spec.permissionType(), grantDir, session.getWorkspacePath(), permissions) ? PermissionCheck.allowed() : PermissionCheck.needAsk();
            return new PermissionRequirement(spec.permissionType(), grantPath, grantPath, spec.title(), spec.description(), check);
        } catch (RuntimeException e) {
            PermissionCheck check = PermissionCheck.error(ToolPermissionErrorCode.PERMISSION_INVALID_PATH, e.getMessage());
            return new PermissionRequirement(spec.permissionType(), null, null, spec.title(), spec.description(), check);
        }
    }
