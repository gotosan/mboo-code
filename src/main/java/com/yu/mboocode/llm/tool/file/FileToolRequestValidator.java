...
@Component
public class FileToolRequestValidator {
    ...
    private JSONObject parseArguments(String json) {
        try {
            ...
            if (value == null) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "工具参数必须是 JSON 对象");
            return value;
        } catch (FileToolException e) {
            throw e;
        } catch (Exception e) {
            throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "工具参数 JSON 格式错误", e);
        }
    }

    private void validateGlob(JSONObject arguments) {
        String pattern = requiredString(arguments, "pattern", false);
        if (pattern.length() > 1024) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "pattern 长度不能超过 1024 个字符");
    ...
    private void validateSearch(JSONObject arguments) {
        ...
        if (query.length() > 4096) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "query 长度不能超过 4096 个字符");
        String glob = optionalString(arguments, "glob");
        if (glob != null && glob.length() > 1024) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "glob 长度不能超过 1024 个字符");
    ...
    private void validateEdit(JSONObject arguments) {
        ...
        if (oldText.isEmpty()) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "oldText 不能为空");
        if (oldText.length() > 1024 * 1024) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "oldText 不能超过 1 MiB 字符数据");
        if (newText.length() > 1024 * 1024) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "newText 不能超过 1 MiB 字符数据");
    ...
    private String requiredString(JSONObject arguments, String name, boolean allowEmpty) {
        if (!arguments.containsKey(name) || arguments.get(name) == null) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "缺少参数：" + name);
        Object value = arguments.get(name);
        if (!(value instanceof String text)) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "参数 " + name + " 必须是字符串");
        if (!allowEmpty && text.isBlank()) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "参数 " + name + " 不能为空");
        return text;
    }
    ...
    private void integerRange(JSONObject arguments, String name, int min, int max) {
        ...
        if (!(raw instanceof Number number)) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "参数 " + name + " 必须是整数");
        long value = number.longValue();
        if (number.doubleValue() != value || value < min || value > max) throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "参数 " + name + " 超出允许范围");
    }
    ...
    private void booleanType(JSONObject arguments, String name) {
        if (arguments.containsKey(name) && arguments.get(name) != null && !(arguments.get(name) instanceof Boolean)) {
            throw new FileToolException(FileToolErrorCode.INVALID_ARGUMENT, "参数 " + name + " 必须是布尔值");
        }
    }
}
