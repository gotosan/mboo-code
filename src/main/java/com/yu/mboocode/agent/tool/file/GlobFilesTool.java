...
import com.yu.mboocode.agent.tool.dto.FilePathItem;
import com.yu.mboocode.agent.tool.dto.ToolResult;
import com.yu.mboocode.agent.tool.dto.GlobFilesData;
import com.yu.mboocode.agent.tool.ToolCommonErrorCode;
import com.yu.mboocode.agent.tool.permission.PathKind;
...
public class GlobFilesTool {
    ...
    public ToolResult<GlobFilesData> glob_files(
        ...
        List<FilePathItem> files = matched.stream().limit(limit)
                .map(file -> new FilePathItem(file.toString(), com.yu.mboocode.agent.tool.permission.FilePermissionUtil.workspaceRelativePath(file, paths.workspace())))
                .toList();
