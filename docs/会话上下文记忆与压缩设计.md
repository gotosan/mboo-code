...
> 本文档已被[《模型级 Token 预算与分层会话上下文设计》](./模型级Token预算与分层会话上下文设计.md)替代，仅保留为历史设计参考。后续实现以新文档为准，本文中的固定 `256K` 窗口、单一旧版压缩流程和实施阶段不再作为当前决策。
...
本文档建立在 [Agent Session JSONL 事件日志设计](./会话JSONL事件日志设计.md) 之上，不改变 JSONL 作为完整历史来源的定位。
...
- JSONL/SSE 中普通工具结果最多保留 2,000 字符，五个文件工具结果最多保留 4,000 字符；该事件上限不影响 LangChain4j 在当前工具循环中使用完整模型结果。
...
当前已注册文件工具和命令工具。上述 `T`、`G` 仍作为第一版固定预留；后续继续增加 Git、MCP 等工具，或调整现有工具 Schema 和结果上限时，需要重新核对预留是否充足。
