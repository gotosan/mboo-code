...
- `TOOL_CALL_ENDED.resultPreview` 最多 4,000 字符。
- 继续由现有 `ToolEventFormatterRegistry` 生成事件摘要，并调用同一个 `ToolTextTruncator` 生成中间截断预览。
- 事件预览保留退出码、工作目录、截断状态和输出头尾。
- JSONL 与 SSE 使用同一份事件内容。
- 前端不进行第二次字符裁剪。
...
3. 后端生成的 `resultPreview`，其中包含退出码、Shell、工作目录、耗时、裁剪状态和输出。
...
`ToolEventFormatterRegistry` 在后端把命令结构化结果格式化为现有 `resultPreview`。前端继续使用通用纯文本结果容器：
