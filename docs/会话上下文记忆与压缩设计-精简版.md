...
> 本文档已被[《模型级 Token 预算与分层会话上下文设计》](./模型级Token预算与分层会话上下文设计.md)替代，仅保留为历史设计参考。后续实现以新文档为准，本文中的固定 `256K` 预算和纯文本滚动摘要不再作为当前决策。

## 文档定位

本文档是 `会话上下文记忆与压缩设计.md` 的第一版精简实施方案，原文档保留作为完整设计参考。
