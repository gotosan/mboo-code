...
export const AskCard = memo(function AskCard({ toolCall, sessionId, isCancelling, onCancel, onProgress }: { toolCall: ToolCallView; sessionId: string; isCancelling: boolean; onCancel: () => void; onProgress: (toolCallId: string, progress: AskDraftProgress) => void }) {
  ...
  return <section className="rounded-xl border border-accent/40 bg-panel p-4 shadow-panel" aria-label="提问" aria-busy={isCancelling}>
    <header className="mb-3 flex items-start justify-between gap-3"><div><p className="text-xs text-text-3">问题 {page + 1}/{questions.length}</p><h3 className="mt-1 whitespace-pre-wrap text-base font-semibold text-text-1">{current.question}</h3></div><div className="flex items-center gap-2"><button type="button" disabled={page === 0 || submitting || isCancelling} onClick={() => { const targetPage = Math.max(0, page - 1); setPage(targetPage); onProgress(toolCall.id, { answers, pageIndex: targetPage }); }} className="rounded border border-line px-2 py-1 text-xs text-text-2 disabled:opacity-40">上一页</button><button type="button" disabled={isCancelling} aria-label="关闭提问并取消当前任务" onClick={onCancel} className="rounded p-1 text-text-3 hover:bg-panel-muted hover:text-text-1 disabled:opacity-40"><X className="size-4" aria-hidden /></button></div></header>
    <div className="space-y-2">{current.answers.map((option, index) => { const selected = option.text === currentAnswer; return <button key={`${option.text}-${index}`} type="button" aria-pressed={selected} disabled={submitting || isCancelling} onClick={() => void submit("ANSWER", option.text)} className={`w-full rounded-lg border px-3 py-2 text-left disabled:opacity-50 ${selected ? "border-accent bg-accent-soft" : "border-line bg-panel-muted hover:border-accent"}`}><span className="flex items-center gap-2 text-sm font-medium">{option.text}{index === 0 ? <span className="rounded bg-accent/15 px-1.5 py-0.5 text-[11px] text-accent">推荐</span> : null}{selected ? <span className="rounded bg-accent px-1.5 py-0.5 text-[11px] text-accent-fg">已选择</span> : null}</span>{option.description ? <span className="mt-1 block whitespace-pre-wrap text-xs leading-5 text-text-3">{option.description}</span> : null}</button>; })}</div>
    {error ? <p className="mt-2 text-xs text-danger" role="alert">{error}</p> : null}<div className="mt-3 flex items-end gap-2"><textarea value={custom} onChange={(event) => setCustom(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit("ANSWER", custom); } }} disabled={submitting || isCancelling} maxLength={2000} rows={2} placeholder="或者输入自定义答案" className={`min-h-10 flex-1 resize-y rounded-lg border px-3 py-2 text-sm outline-none focus:border-accent ${hasCustomAnswer ? "border-accent bg-accent-soft" : "border-line bg-panel-muted"}`} aria-label={hasCustomAnswer ? "自定义答案，当前已选择" : "自定义答案"} /><button type="button" aria-pressed={skipped} disabled={submitting || isCancelling} onClick={() => void submit("SKIP")} className={`rounded-lg border px-3 py-2 text-sm disabled:opacity-50 ${skipped ? "border-accent bg-accent-soft text-accent-strong" : "border-line text-text-2 hover:bg-panel-muted"}`}>{skipped ? "已跳过" : "跳过"}</button></div>
  </section>;
});
